                ...
                if (lockRecv)
                {
                    ...
                    if (nPos > ReceiveBufferSize()) {
                        ...
                        else if (nBytes == 0)
                        {
                            ...
                            if (!pnode->fDisconnect)
                                printf("socket closed - ThreadSocketHandler2\n");
                            pnode->CloseSocketDisconnect();
                        }
                        else if (nBytes < 0)
                        {
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Coin2.1 " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"Coin2Main", "107.181.166.143"},
    {"Coin2Node1", "24.22.50.162"},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Coin2.1 is probably already running."), addrBind.ToString().c_str());
        else
