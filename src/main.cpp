...
const string strMessageMagic = "Coin2.1 Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Coin2.1
    bool fStrictPayToScriptHash = true; // Always active in Coin2.1
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    nTransactionsUpdated++;
    printf("SetBestChain: new best=%s  height=%d  trust=%s  date=%s Stake checkpoint: %x\n",
      hashBestChain.ToString().c_str(), nBestHeight, bnBestChainTrust.ToString().c_str(),
      DateTimeStrFormat("%x %H:%M:%S", pindexBest->GetBlockTime()).c_str(), pindexBest->nStakeModifierChecksum);
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Coin2.1", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
FILE* AppendBlockFile(unsigned int& nFileRet)
{
    ...
    loop()
    {
        ...
        // ~100MB
        if (ftell(file) < (long)(0x08000000 - MAX_SIZE))
        {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        printf("partner %s using version %i\n", pfrom->addr.ToString().c_str(), pfrom->nVersion);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    pblock->print();
    printf("proof-of-work generated %s\n", FormatMoney(pblock->vtx[0].vout[0].nValue).c_str());
